...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Data Futures Platform pipeline command line script.",
    ...
    subparsers = parser.add_subparsers(
        help="main commands", dest="command", required=True
    )
    list_parser = subparsers.add_parser(
        name="list",
        help="List pipeline info and/or configuration",
        description="List pipeline info and/or configuration ",
    )
    list_parser.add_argument(
        "-i", "--indicators", help="List available indicators", action="store_true"
    )
    list_parser.add_argument(
        "-s", "--sources", help="List available sources", action="store_true"
    )
    list_parser.add_argument(
        "-c", "--config", help="List available configuration", action="store_true"
    )
    ...
    try:
        if args.command == "list":
            ...
            await list_command(
                indicators=args.indicators, sources=args.sources, config=args.config
            )
        if args.command == "run":
            validate_env()
            ...
            if args.stage is None:
                ...
                transformed_indicators = await transform_sources(
                    indicator_ids=downloaded_indicators, project="access_all_data"
                )
                published_indicators = await publish(
                    indicator_ids=transformed_indicators, project="access_all_data"
                )
