...
async def publish_indicator(
    ...
    try:
        ...
        if len(years) == 0:
            ...
            raise PublishError(
                f"No publish-able data found for indicator_id {indicator_id}"
            )

        for year in years:
            # Extract data for the specific year and construct the year_df
            year_df = base_df[
                [constants.STANDARD_KEY_COLUMN] + [f"{indicator_id}_{year}"]
            ]
        ...
        indicator_df.rename(
            columns={constants.STANDARD_KEY_COLUMN: "country_iso3"}, inplace=True
        )
        ...
        with open(
            f"/home/thuha/Desktop/UNDP/dfp/dv-data-pipeline/outputs/{indicator_id}.json",
            "w",
        ) as f:
