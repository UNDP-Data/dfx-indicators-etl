...
def transform_series(df: pd.DataFrame, iso_3_map: dict) -> pd.DataFrame:
    ...
    df_country.rename(
        columns={"id": "alpha_3_code", "value": "country_name"}, inplace=True
    )
    ...
    df.rename(columns=PRIMARY_COLUMNS_TO_RENAME, inplace=True)
    ...
    try:
        ...
        logging.warning("Could not convert year to integer type")

    # Replace empty strings with None
    df.replace({"": None}, inplace=True)

    # Filter rows based on alpha_3_code presence in iso_3_map
    df = df.loc[df["alpha_3_code"].isin(iso_3_map.keys())].reset_index(drop=True)
