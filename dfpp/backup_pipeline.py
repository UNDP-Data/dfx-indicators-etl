    ...
    try:
        ...
        async with StorageManager(
            ...
            if indicator_ids and len(indicator_ids) > 0:
                source_ids = list(set([indicator_cfg.get('indicator').get('source_id') for indicator_cfg in
                                       await storage_manager.get_indicators_cfg(indicator_ids=indicator_ids)]))
            else:
                source_ids = list(set([indicator_cfg.get('indicator').get('source_id') for indicator_cfg in
                                       await storage_manager.get_indicators_cfg()]))
                save_as = [source_cfg.get('source').get('save_as') for source_cfg in
                           await storage_manager.get_sources_cfgs(source_ids=source_ids)]
            raw_sources_list = [f'{storage_manager.SOURCES_PATH}/{save_as[i]}' for
                                i, source_id in enumerate(source_ids)]
            ...
            for raw_source_path in raw_sources_list:
                ...
                raw_backup_source_blob_name = raw_source_path.replace(storage_manager.SOURCES_PATH,
                                                                      os.path.join(storage_manager.BACKUP_PATH,
                                                                                   'sources'))
                await storage_manager.copy_blob(source_blob_name=raw_source_path,
                                                destination_blob_name=raw_backup_source_blob_name)
                logger.info(f'Created a backup for {raw_source_path} to {raw_backup_source_blob_name}')
    ...
    try:
        ...
        async with StorageManager(
            ...
            async def get_base_files_list(indicator_ids=None):
                ...
                source_ids = list(set(
                    cfg.get('indicator').get('source_id') for cfg in indicator_cfgs
                ))
                base_files = [
            ...
            for base_file_path in base_files_list:
                try:
                    ...
                    logger.info(f'Created a backup for {base_file_path} to {base_backup_file_blob_name}')
