...
from dfpp.transformation.column_name_template import DIMENSION_COLUMN_PREFIX, DIMENSION_COLUMN_CODE_SUFFIX, DIMENSION_COLUMN_NAME_SUFFIX, SexEnum, sort_columns_canonically
...
def transform_series(
    ...
    for column in dimension_columns:
        ...
        if column_name_formatted == "age":
            df[f"{DIMENSION_COLUMN_PREFIX}{column_name_formatted}{DIMENSION_COLUMN_CODE_SUFFIX}"] = df[column]
            df[f"{DIMENSION_COLUMN_PREFIX}{column_name_formatted}{DIMENSION_COLUMN_NAME_SUFFIX}"] = df[column].replace(AGE_REMAP)
            continue
        if column_name_formatted == "sex":
            df[f"{DIMENSION_COLUMN_PREFIX}{column_name_formatted}{DIMENSION_COLUMN_CODE_SUFFIX}"] = df[column]
            df[f"{DIMENSION_COLUMN_PREFIX}{column_name_formatted}{DIMENSION_COLUMN_NAME_SUFFIX}"] = df[column].replace(SEX_REMAP)
            continue

        df[f"{DIMENSION_COLUMN_PREFIX}{column_name_formatted}{DIMENSION_COLUMN_CODE_SUFFIX}"] = df[column]
        df[f"{DIMENSION_COLUMN_PREFIX}{column_name_formatted}{DIMENSION_COLUMN_NAME_SUFFIX}"] = df[column].replace(to_remap)
    ...
    disagr_columns = [col for col in df.columns if col.startswith(DIMENSION_COLUMN_PREFIX)]
