...
def sort_columns_canonically(df):
    ...
    other_cols = [col for col in df.columns if col not in CANONICAL_COLUMN_NAMES and col not in grouped_disagr_cols]

    sorted_columns = canonical_cols_start + grouped_disagr_cols + other_cols + canonical_cols_end
...
def get_grouped_disagr_columns(df: pd.DataFrame) -> list:
    ...
    disagr_cols = [col for col in df.columns if col.startswith("disagr_")]
    if not disagr_cols:
        return []
