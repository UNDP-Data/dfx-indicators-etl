...
async def run_transformation_for_indicator(
    ...
    try:
        ...
        # tbd: run transformation and return dataframe
        await run_transform(
            source_df=source_df,
            indicator_id=indicator_id,
        ...
        # upload to blob storage separately
    except Exception as e:
        logger.error(e)
        raise e
    return indicator_id
...
async def process_indicator(indicator_cfg: dict, semaphore: asyncio.Semaphore,
    ...
    async with semaphore:
        try:
            ...
            logger.error(f"Error occurred while processing {indicator_id}: {str(e)}")
            processing_statuses["failed_indicators"].append(indicator_id)
...
async def transform_sources(
    ...
    """
    processing_statuses = {"failed_indicators": [],
    ...
    async with StorageManager() as storage_manager:
        ...
        tasks = [process_indicator(cfg, semaphore, processing_statuses) for cfg in indicators_cfgs]
        results = await asyncio.gather(*tasks)

        logger.info("#" * 100)
        logger.info(f"TASKED: {len(indicators_cfgs)} indicators")
        logger.info(f"TRANSFORMED: {len(processing_statuses['transformed_indicators'])} indicators")
        logger.info(f"FAILED: {len(processing_statuses['failed_indicators'])} indicators")
        logger.info("#" * 100)

        return processing_statuses["transformed_indicators"] if processing_statuses["transformed_indicators"] else None
