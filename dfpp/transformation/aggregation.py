...
async def region_population_mapping(population_df: pd.DataFrame, grouped_region: Any):
    ...
    try:
        ...
        for index, row in population_df.iterrows():
            ...
            co_df["year"] = co_df["index"].apply(
                lambda date_string: date_string.replace("totalpopulation_untp_", "")
            )
            co_df = co_df.iloc[1:]

            # Interpolate data for population values
            pop_years, pod_values, cleaned_df = await interpolate_data(
                data_frame=co_df[["year", index]], target_column=index
            )
        ...
        for group_name, df_group in grouped_region:
            ...
            for year in range(2000, CURRENT_YEAR):
                if f"totalpopulation_untp_{year}" in population_df.columns:
                    region_population[f"{group_name}_{str(year)}"] = population_df[
                        population_df.index.isin(country_list)
                    ][f"totalpopulation_untp_{str(year)}"].sum()
                else:
                    logger.info(
                        f"Not found totalpopulation_untp_{str(year)}, {group_name}"
                    )
...
async def generate_region_aggregates(
    ...
    try:
        ...
        indicator_configurations = await storage_manager.get_indicators_cfg(
        ...
        if not indicator_config["aggregate_type"]:
            logger.warning(
                f"Skipping indicator {indicator_id} with missing aggregate_type"
            )
            return None
        ...
        if denominator_indicator_name != "None" and denominator_indicator_name != "":
            denominator_column_prefix = (
                "^" + re.escape(denominator_indicator_name) + "_2"
            )
            denominator_base_df = await base_df_for_indicator(
                storage_manager=storage_manager,
                indicator_id=denominator_indicator_name,
                project=project,
            )
            denominator_base_df.set_index(STANDARD_KEY_COLUMN, inplace=True)
            denominator_base_df = denominator_base_df.drop(columns=["index"])
            denominator_df = denominator_base_df.filter(
                regex=denominator_column_prefix, axis=1
            )
        if (
            indicator_config["min_year"] != "None"
            and indicator_config["min_year"] != ""
        ):
            minimum_year = str(indicator_config["min_year"])

        base_file_df = await base_df_for_indicator(
            storage_manager=storage_manager, indicator_id=indicator_id, project=project
        )
        ...
        for group_name, region_df in grouped_region:
            ...
            region_indicator_df = indicator_df[
                base_file_df.index.isin(country_iso3_list)
            ]
            region_indicator_df = region_indicator_df.reindex(
                sorted(region_indicator_df.columns), axis=1
            )

            if denominator_df is not None:
                region_denominator_df = denominator_df[
                    denominator_base_df.index.isin(country_iso3_list)
                ]
                region_denominator_df = region_denominator_df.reindex(
                    sorted(region_denominator_df.columns), axis=1
                )
            ...
            for year in range(2000, CURRENT_YEAR):
                ...
                if (
                    denominator_df is not None
                    and (denominator_indicator_name + "_" + str(year))
                    not in region_denominator_df.columns
                ):
                    region_denominator_df[
                        denominator_indicator_name + "_" + str(year)
                    ] = np.NaN

            # Transform data for further processing
            transformed_df = region_indicator_df.transpose()
            transformed_df.reset_index(inplace=True)
            transformed_df["year"] = transformed_df["index"].apply(
                lambda date_string: date_string.replace(indicator_id + "_", "")
            )
            ...
            if region_denominator_df is not None:
                ...
                denominator_transformed_df["year"] = denominator_transformed_df[
                    "index"
                ].apply(
                    lambda date_string: date_string.replace(
                        denominator_indicator_name + "_", ""
                    )
                )
            ...
            for col in transformed_df.columns:
                if col != "year" and col != "index":
                    ...
                    denominator_values = {}
                    if denominator_transformed_df is not None:
                        denominator_year_col_df = denominator_transformed_df[
                            ["year", col]
                        ]
                        interpolated_years_d, interpolated_values_d, cleaned_df_d = (
                            await interpolate_data(
                        ...
                        for i, interpolated_value_d in enumerate(interpolated_values_d):
                            denominator_values[interpolated_years_d[i]] = pd.to_numeric(
                                interpolated_value_d
                            )
                            denominator_year_col_df.loc[interpolated_years_d[i]] = (
                                pd.to_numeric(interpolated_value_d)
                            )

                        year_col_df.set_index("year", inplace=True)
                        transformed_indicator_df = year_col_df.select_dtypes(
                            exclude=["object", "datetime"]
                        ) * float(per_capita_factor)
                        transformed_indicator_df.index = pd.to_numeric(
                            transformed_indicator_df.index
                        )
                        year_col_df = transformed_indicator_df.mul(
                            denominator_year_col_df, axis=0
                        )
                        year_col_df.reset_index(inplace=True)

                    interpolated_years, interpolated_values, cleaned_df = (
                        await interpolate_data(year_col_df, col, minimum_year)
                    )
                    for i2, row2 in cleaned_df.iterrows():
                        if (
                            total_original_data_count.get(
                                group_name + "_" + str(row2["year"])
                            )
                            is None
                        ):
                            total_original_data_count[
                    ...
                    for i, interpolated_year in enumerate(interpolated_years):
                        ...
                        if (
                            indicator_data.get(
                                indicator_id + "_" + str(interpolated_year)
                            )
                            is None
                        ):
                            indicator_data[
                        ...
                        if (
                            denominator_data is not None
                            and denominator_data.get(
                                indicator_id + "_" + str(interpolated_year)
                            )
                            is None
                        ):
                        ...
                        if (
                            indicator_config["aggregate_type"] == "PositiveSum"
                            and interpolated_values[i] <= 0
                        ):
                            interpolated_values[i] = 0
                            if denominator_data is not None:
                                denominator_data[
                                    indicator_id + "_" + str(interpolated_year)
                                ] += denominator_values.get(interpolated_year)
                        ...
                        individual_country_data_list.append(
                            {
                                ...
                                "denominator": denominator_values.get(
                                    interpolated_year
                                ),
                            }
                        )
                        ...
                        if denominator_data[denominator_key] == 0:
                            ...
                            indicator_data[denominator_key] = round(
                                (
                                    ...
                                    / (
                                        denominator_data[denominator_key]
                                        * per_capita_factor
                                    )
                                ),
                                5,
            ...
            for indicator_key in indicator_data:
                ...
                original_data_count = total_original_data_count.get(
                    group_name + "_" + year_str, 0
                )

                if total_population == 0:
                    logger.info(
                        f"Population is zero for {indicator_id}, {year_str}, {group_name}"
                    )
                    continue
        ...
        if len(aggregated_data_list) == 0:
            raise AggregationError(
                f"No aggregated data found for indicator_id {indicator_id}"
            )
...
async def aggregate_indicator(project: str = None, indicator_id: str = None):
    ...
    try:
        ...
        async with StorageManager() as storage_manager:
            ...
            region_population = await region_population_mapping(
                population_df=population_df, grouped_region=grouped_region
            )

            # Generate aggregated data for regions
            aggregated_data_df, individual_country_data_df = (
                await generate_region_aggregates(
                    storage_manager=storage_manager,
            ...
            new_col = "ind_y"
            aggregated_data_df[new_col] = (
                aggregated_data_df["indicatorid"] + "_" + aggregated_data_df["year"]
            )

            # Pivot the data to a new format using 'region' and 'ind_y' as indices
            aggregate_df = aggregated_data_df.pivot(
                index="region", columns="ind_y", values="final_value"
            )
            ...
            indicator_columns = [
                column for column in aggregate_df.columns if indicator_id in column
            ]
            region_indicator_df = pd.DataFrame(
                columns=["indicator_id", "region", "year", "value"]
            )
            ...
            for year in years:
                ...
                logger.info(
                    f"Completed aggregation process for indicator_id {indicator_id}"
                )
