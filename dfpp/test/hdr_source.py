...
async def run():
    async with StorageManager() as sm:
        an_ind = await sm.get_indicator_cfg(
            indicator_id="expected_years_of_schooling_female_hdr"
        )
...
async def inspect():
    async with StorageManager(clear_cache=True) as sm:
        ...
        for indicator_cfg in indicators_cfg:
            ...
            if source_cfg:
                if (
                    "downloader_params" in source_cfg
                    and source_cfg["downloader_params"]
                ):
                    downloader_params = source_cfg["downloader_params"]
                    if "request_params" in downloader_params:
                        params_str = json.loads(
                            source_cfg["downloader_params"]["request_params"]
                        )["value"]
